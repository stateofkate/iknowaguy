...
class LlmClient:
    ...
    def prepare_prompt(self, request: CustomLlmRequest):
        prompt = [{
          ...
          agent_prompt.format(quantity=self.call_info['quantity'], 
                              sku_id=self.call_info['sku_id'], 
                              vendor=self.call_info['vendor'],
                            best_quote=self.call_info['best_quote']
                              )
        }]
    ...
    @agentops.record_function('draft_response')
